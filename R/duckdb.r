...
#'   large, unpartitioned datasets (e.g. Veraset Visits). Defaults to 25. Lower it if
#'   you still hit request errors; raise it for throughput on small, reliable datasets.
#'
...
#'
#' @export
download_dewey_duck <- function(api_key, data_id, output_dir = get_download_dir(), partition, overwrite = FALSE, file_name = NULL, where = NULL, select = NULL, batch_size = 25) {
  ...
  # overwhelms the download API (HTTP 500), which is what breaks large, unpartitioned
  # datasets like Veraset Visits. Batching bounds how many remote files are open at a
  # time; each batch writes with a unique filename so batches never overwrite each other.
